package com.dawnlight.nvstream.mdns;
...
public abstract class MdnsDiscoveryAgent {
    ...
    protected void reportNewComputer(String name, int port, Inet4Address[] v4Addrs, Inet6Address[] v6Addrs) {
        AppLog.info("mDNS: "+name+" has "+v4Addrs.length+" IPv4 addresses");
        AppLog.info("mDNS: "+name+" has "+v6Addrs.length+" IPv6 addresses");
    ...
    protected static Inet6Address getLinkLocalAddress(Inet6Address[] addresses) {
        for (Inet6Address addr : addresses) {
            if (addr.isLinkLocalAddress()) {
                AppLog.info("Found link-local address: "+addr.getHostAddress());
                return addr;
            }
        }
        ...
        for (int tries = 0; tries < 2; tries++) {
            ...
            for (Inet6Address addr : addresses) {
                if (addr.isLinkLocalAddress() || addr.isSiteLocalAddress() || addr.isLoopbackAddress()) {
                    ...
                    AppLog.info("Ignoring non-global address: "+addr.getHostAddress());
                    continue;
                }
                ...
                if (addrBytes[0] == 0x20 && addrBytes[1] == 0x02) {
                    ...
                    AppLog.info("Ignoring 6to4 address: "+addr.getHostAddress());
                    continue;
                }
                // 2001::/32
                else if (addrBytes[0] == 0x20 && addrBytes[1] == 0x01 && addrBytes[2] == 0x00 && addrBytes[3] == 0x00) {
                    // Teredo also has horrible performance
                    AppLog.info("Ignoring Teredo address: "+addr.getHostAddress());
                    continue;
                }
                // fc00::/7
                else if ((addrBytes[0] & 0xfe) == 0xfc) {
                    // ULAs aren't global
                    AppLog.info("Ignoring ULA: "+addr.getHostAddress());
                    continue;
                }
                ...
                if (linkLocalAddr != null && tries == 0) {
                    ...
                    if (!matched) {
                        AppLog.info("Ignoring non-matching global address: "+addr.getHostAddress());
                        continue;
                    }
                }
